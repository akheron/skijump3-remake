...
impl<'a> SDLPortModule<'a> {
    ...
    pub fn render_phase3(&self) -> RenderResult {
        if !self.frame_requested.get() {
            return RenderResult::Skipped;
        }
        if !self.frame_duration_passed() {
            return RenderResult::Wait;
        }
    ...
    pub fn frame_duration_passed(&self) -> bool {
    ...
    fn process_keypresses(&self) -> Option<(u8, u8)> {
        ...
        let mut keypresses = self.keypresses.borrow_mut();
        while keypresses.len() > 0 {
            let (mut keycode, keymod) = keypresses.remove(0);
            /*
    ...
    pub async fn wait_for_key_press(&self) -> (u8, u8) {
        return WaitForKeypressFuture { s: self }.await;
    }
    ...
    pub async fn wait_for_key(&self) {
        self.putsaa();
        self.wait_for_key_press().await;
    }

    pub async fn wait_for_key2(&self) -> bool {
        self.putsaa();
...
struct WaitForKeypressFuture<'s, 'si> {
    s: &'s SDLPortModule<'si>,
}

impl<'s, 'si> Future for WaitForKeypressFuture<'s, 'si> {
    type Output = (u8, u8);

    fn poll(self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Self::Output> {
        if let Some((ch, ch2)) = self.s.process_keypresses() {
            self.s.ch.set(ch);
            self.s.ch2.set(ch2);
            Poll::Ready((ch, ch2))
        } else {
